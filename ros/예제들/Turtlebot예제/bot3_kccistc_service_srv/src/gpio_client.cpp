...
{
  ros::init(argc, argv, "gpio_client");    // Initializes Node Name

  if (argc != 3)  // Input value error handling
  {
    ROS_INFO("cmd : rosrun bot3_kccistc_service gpio_client arg0 arg1");
  ...
  ros::ServiceClient bot3_kccistc_service_client = nh.serviceClient<bot3_kccistc_service::bot3gpio>("bot3_gpio_srv");
  ...
  if (bot3_kccistc_service_client.call(srv))
  {
  ...
  {
    ROS_ERROR("Failed to call service bot3_kccistc_srv");
    return 1;
  }
  return 0;
}
