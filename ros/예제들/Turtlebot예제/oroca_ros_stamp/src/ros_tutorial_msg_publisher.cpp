...
{
  ros::init(argc, argv, "ros_tutorial_msg_publisher_stamp");  // 노드명 초기화
  ...
  ros::Publisher ros_tutorial_pub = nh.advertise<oroca_ros_stamp::msgTutorial>("ros_tutorial_msg_stamp", 100);
  ...
  oroca_ros_stamp::msgTutorial msg;      // msgTutorial 메시지 파일 형식으로 msg 라는 메시지를 선언
  while (ros::ok())
  {
	...
	ROS_INFO("send string = %s", msg.str.c_str());
