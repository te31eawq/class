...
#include "oroca_ros_stamp/msgTutorial.h"            // msgTutorial 메시지 파일 헤더 (빌드후 자동 생성됨)

// 메시지 콜백함수로써, 밑에서 설정한 ros_tutorial_sub 구독자에 해당되는 메시지를
// 수신하였을때 동작하는 함수이다
// 입력 메시지로는 oroca_ros_tutorial 패키지의 msgTutorial 메시지를 받도록 되어있다
void msgCallback(const oroca_ros_stamp::msgTutorial::ConstPtr& msg)
{
  ROS_INFO("recieve msg: %d", msg->data);   // 수신된 메시지를 표시하는 함수
  ROS_INFO("recieve sec: %d", msg->stamp.sec);
  ROS_INFO("recieve nsec: %d", msg->stamp.nsec);
  ROS_INFO("recieve string: %s", msg->str.c_str());
}

int main(int argc, char **argv)                         // 노드 메인 함수
{
  ros::init(argc, argv, "ros_tutorial_msg_subscriber_stamp"); // 노드명 초기화
  ...
  ros::Subscriber ros_tutorial_sub = nh.subscribe("ros_tutorial_msg_stamp", 100, msgCallback);
