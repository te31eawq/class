...
void loop() {
  ...
  if (timerIsrFlag) //1초에 한번씩 실행
  {
    ...
    if (!(secCount % 5)) //5초에 한번씩 실행
    {
      ...
      float correctedPPM = gasSensor.getCorrectedPPM(temp, humi);  // 온도, 습도를 고려한 PPM 값
      
      // cds = map(correctedPPM, 0, 1023, 0, 100);
      cds = correctedPPM;
      moterpwm = map(correctedPPM, 0, 1023, 0, 255);

      analogWrite(MOTOR_PIN, moterpwm);  // 모터의 속도를 설정 (PWM)
      if ((cds >= 70) && cdsFlag)
      {
        cdsFlag = false;
